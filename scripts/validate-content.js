...
// Configuration
const CONTENT_BUNDLE = path.resolve(
  __dirname,
  "../src/generated/content-bundle.ts",
);
...
if (!fs.existsSync(CONTENT_BUNDLE)) {
  console.error(
    '❌ Content bundle not found. Run "npm run build:content" first.',
  );
  process.exit(1);
}
...
function validateContentIntegrity(bundleData) {
  ...
  if (fs.existsSync(PRIVATE_CONTENT)) {
    const privateProjects = fs
      .readdirSync(PRIVATE_CONTENT, { withFileTypes: true })
      .filter((dirent) => dirent.isDirectory())
      .map((dirent) => dirent.name);

    console.log(
      `📁 Private content source: ${privateProjects.length} projects found`,
    );
...
function performSecurityChecks(bundleData) {
  ...
  for (const pattern of sensitivePatterns) {
    if (pattern.test(bundleData.content)) {
      console.warn(
        `⚠️  Potential sensitive data pattern found: ${pattern.source}`,
      );
      securityIssues++;
    }
  }
  ...
  if (bundleSize > 1000000) {
    // 1MB limit
    console.warn(
      `⚠️  Bundle size is large: ${(bundleSize / 1024 / 1024).toFixed(2)}MB`,
    );
    securityIssues++;
  }
