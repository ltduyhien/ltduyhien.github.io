...
export class AntiScrapingProtection {
  ...
  detectScraping(req: Request, clientIP?: string): ScrapingDetection {
    ...
    const recommendations = this.generateRecommendations(isBot, isScraper, riskLevel);
  ...
  private analyzeBehavior(clientIP: string): { suspicious: boolean; reason: string } {
    const now = Date.now();
    ...
    history.timestamps = history.timestamps.filter(timestamp => timestamp > oneMinuteAgo);
    ...
    if (history.timestamps.length >= 2) {
      const timeDiff = history.timestamps[history.timestamps.length - 1] - history.timestamps[history.timestamps.length - 2];
  ...
  private generateRecommendations(isBot: boolean, isScraper: boolean, riskLevel: 'low' | 'medium' | 'high'): string[] {
  ...
  // Get statistics
  getStats(): { totalIPs: number; suspiciousIPs: number; requestHistorySize: number } {
    return {
