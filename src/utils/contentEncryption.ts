...
class ContentEncryption {
  ...
  private startKeyRotation(): void {
    this.keyRotationInterval = setInterval(() => {
      this.rotateKey();
    }, 25 * 60 * 1000); // Rotate every 25 minutes (before 30 min expiry)
  }
  ...
  public async encryptContent(content: string): Promise<string> {
    ...
    try {
      ...
      const keyBuffer = new Uint8Array(this.currentKey.key.match(/.{1,2}/g)!.map(byte => parseInt(byte, 16)));
  ...
  public async decryptContent(encryptedData: string): Promise<string> {
    ...
    try {
      ...
      const encryptedBuffer = new Uint8Array(encryptedContent.data.match(/.{1,2}/g)!.map(byte => parseInt(byte, 16)));
      const iv = new Uint8Array(encryptedContent.iv.match(/.{1,2}/g)!.map(byte => parseInt(byte, 16)));
  ...
  private async verifyWatermark(encryptedContent: EncryptedContent): Promise<boolean> {
    try {
      const watermarkData: WatermarkData = JSON.parse(atob(encryptedContent.watermark));
  ...
  private async createSignature(data: string, watermark: string): Promise<string> {
  ...
  private async verifyPageWatermarks(): Promise<void> {
    ...
    if (verificationFailures > 0) {
      console.warn(`🚨 ${verificationFailures} watermark verification failures detected`);
      this.emitSecurityAlert('watermark_violation', { failures: verificationFailures });
    }
  }
  ...
  private emitSecurityAlert(type: string, data: any): void {
